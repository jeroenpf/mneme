...
Local AI dev knowledge service. Go + PostgreSQL + Vue3, runs locally on the laptop under Docker Compose, reached at `https://mneme.dev:8443` (`mneme.dev` → `127.0.0.1` via `/etc/hosts`; the Go server terminates TLS with an mkcert cert, no reverse proxy). Exposes an MCP server so Codex can push/query documents. The host is `mneme.dev`, **not** `mneme.local` — macOS routes `*.local` through mDNS, which stalls resolution ~5s before reading `/etc/hosts`.
...
- **Local-only by design** — reachable only from this Mac. Phone/iPad/other devices can't hit `mneme.local`. Acceptable: the consumer is Codex on the laptop.
