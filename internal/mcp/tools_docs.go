...
type pushInput struct {
	Meta      map[string]any `json:"meta" jsonschema:"document meta: id (required stable slug), title (required), type, project, status, tags, phase_current/phase_total — the full key reference is in the server instructions"`
	Body      map[string]any `json:"body" jsonschema:"body block tree {sections:[...]} — block shapes and prose rules are in the server instructions; omitted ids are minted and listed in 'created'"`
